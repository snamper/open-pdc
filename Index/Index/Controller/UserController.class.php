...
/*            if($temp_size[0] < 100 || $temp_size[1] < 100){//判断宽和高是否符合头像要求
    ...
    public function UpAvatar(){
        ...
        if(!isset($params) || empty($params)){
            $this->ajaxReturn(['status' => 1, 'message' => '参数错误！']);
        }
        $params['width'] = (float)$params['picw'];
        $params['height'] = (float)$params['pich'];
        //头像目录地址
        $path = ROOT_PATH . '/Public/Avatar/';
        $path2 = './Public/Avatar/';
        //保存到数据库中
        $UserSystemData['data']['avatar'] = $path2.$UserSystemData['uid'];
        $Avatar = CJson($UserSystemData['data']);
        $User = M('User')->where("uid = '{$UserSystemData['uid']}'")->setField('Data',$Avatar);
        if(!$User and $User !== 0){
            $this->ajaxReturn(['status' => 1, 'message' => '保存数据失败！']);
            exit;
        }
        //要保存的图片
        ...
        $tempimg = $Think_img->open($pic_path);
        //裁剪原图得到选中区域
        $width = $tempimg->width();
        $height = $tempimg->height();
        if($width > 2048 || $height > 2048){
            $this->ajaxReturn(['status' => 1, 'message' => '图片分辨率过高!']);
        }
        $tempimg->crop($params['width'],$params['height'],$params['x'],$params['y'])->save($real_path);
        //生成缩略图
        ...
        $this->ajaxReturn(['status' => 0, 'message' => '上传头像成功！']);
    }
