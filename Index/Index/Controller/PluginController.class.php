...
class PluginController extends UserloginController {
    ...
    protected function GetShop($id){
        ...
        $this->title .= $Plug['title'];
		if(($Plug['pluginstate'] == 1 && $Plug['avaliable'] != 1) || $this->UserSystemData['action'] > 9){
			...
			//结束
			$this->display('plugin');
		} elseif($Plug['avaliable'] == 1 || $this->UserSystemData['action'] < 9) {
			$this->error('本插件已下架', U('/Index/Index'));
		} else {
