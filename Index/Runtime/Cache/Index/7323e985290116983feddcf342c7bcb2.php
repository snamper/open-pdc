    ...
    <div class="uk-width-large-8-10">
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
			...
			<div class="uk-panel uk-panel-box uk-panel-box-secondary">
				...
				<div id="mode">
					<label class="radio-inline">
						<input type="radio" id="plustate-1" name="mode2" value="opensource" <?php if($plug['mode']==1) echo'checked';?>><label for="plustate-1"> 开源</label>
					</label>
					<!--<label class="radio-inline">
						<input type="radio" id="plustate-2" name="mode2" value="closesource" <?php if($plug['mode']==2) echo'checked';?>><label for="plustate-2"> 加密</label>
					</label>-->
					<label class="radio-inline">
						<input type="radio" id="plustate-3" name="mode2" value="default" <?php if($plug['mode']==3) echo'checked';?>><label for="plustate-3"> 原文件</label>
					</label>
				</div>
			...
			<div class="uk-panel uk-panel-box uk-panel-box-secondary">
				...
				<div id="mode">
					<label class="radio-inline">
						<input type="radio" id="or-1" name="or" value="0" <?php if($plug['or']==0) echo'checked';?>><label for="or-1"> 原创</label>
					</label>
					<label class="radio-inline">
						<input type="radio" id="or-2" name="or" value="1" <?php if($plug['or']==1) echo'checked';?>><label for="or-2"> 转载</label>
					</label>
				</div>
			</div>
			...
			<div class="uk-panel uk-panel-box uk-panel-box-secondary">
				...
				<div class="uk-form-row">
					<input name="version" type="text" id="version" placeholder="插件版本" value="<?php echo ($plug["version"]); ?>" class="uk-width-large-1-1">
				</div>
			</div>
			
			<div class="uk-panel uk-panel-box uk-panel-box-secondary" style="display: none;" id="updatecontent-div">
				<div class="uk-panel-title">插件更新简介：</div>
				<p>推送更新用的</p>
				<div class="uk-form-row">
					<textarea name="updatecontent" rows="5" class="form-control" check="false"></textarea>
				</div>
			</div>
			...
			<div class="uk-panel uk-panel-box uk-panel-box-secondary">
				...
				<div class="uk-form-row">
					<input name="package" type="text" placeholder="插件主类" value="<?php echo ($plug["package"]); ?>" class="uk-width-large-1-1" check="false">
				</div>
			</div>
			<div class="uk-panel uk-panel-box uk-panel-box-secondary">
				...
				<?php
 foreach($catelogue as $one){ if(strval($one['tid']) != $plug['catelogue']){ echo '<option value="' . $one['tid'] . '">' . $one['content'] . '</option>'; } else { echo '<option value="' . $one['tid'] . '" selected="selected">' . $one['content'] . '</option>'; } }?>
				</select>
				</div>
			</div>
			<div class="uk-panel uk-panel-box uk-panel-box-secondary">
	...
	$('#version').keyup(function(){
		if($('#version').val()!=oldversion){
			$('#updatecontent-div').show();
		} else {
			$('#updatecontent-div').hide();
		}
	});
	
	if(window.localStorage.getItem('pluginCache-<?php echo ($plug["pid"]); ?>') != null && confirm('是否恢复上次未保存的数据？')){
		var data;
		eval('data = '+window.localStorage.getItem('pluginCache-<?php echo ($plug["pid"]); ?>'));
	...
	setInterval(function(){
		...
		data.fid = $('#fileid').val();
		window.localStorage.setItem('pluginCache-<?php echo ($plug["pid"]); ?>', JSON.stringify(data));
	}, 30000);
			...
			$.ajax({
                ...
                success:function(data){
                    if(data.status == '0'){
						...
						window.localStorage.removeItem('pluginCache-<?php echo ($plug["pid"]); ?>');
						setTimeout(function(){
