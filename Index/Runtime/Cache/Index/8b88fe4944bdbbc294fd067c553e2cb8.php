...
<?php if(is_array($PlugData)): $i = 0; $__LIST__ = $PlugData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$Plug): $mod = ($i % 2 );++$i; $Plug['data'] = CJson($Plug['data'],true); if(empty($Plug['thumburl'])){ $Plug['thumburl'] = '/Public/images/mcbackground.jpg'; } if($Plug['or'] == 0){ $Plug['title'] = '[原创] ' . $Plug['title']; } else { $Plug['title'] = '[转载] ' . $Plug['title']; } ?>
<div id="article-list">
<article class="zan-post clearfix">
 <i class="fa fa-bookmark article-stick"></i> 
	<!-- 大型设备文章显示 -->
	<section class="visible-md visible-lg">
    <div class="category-cloud"><a rel="category tag"><?php echo getCatelogue($Plug['catelogue']);?></a></div>
		<h3>
	...
	<section class="visible-xs  visible-sm">
		...
		<p class="post-info">
			<span><i class="fa fa-calendar"></i> <?php echo ($Plug["updatetime"]); ?></span>
			<span><i class="fa fa-comment"></i> <?php echo getPluginComments($Plug['pid']);?>条</span>
			<span><i class="fa fa-tags"></i> <?php echo getCatelogue($Plug['catelogue']);?></span>
		</p>
