                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
									<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$Plug): $mod = ($i % 2 );++$i;?><tr class="odd gradeX" id="pl-<?php echo ($Plug["pid"]); ?>">
                                        ...
                                        <td><a href="<?php echo U('/Plugin/'.$Plug['pid']);?>" target="_blank"><?php echo ($Plug["title"]); ?></a></td>
										<td><?php echo getCanSeeName($Plug);?></td>
										<td>
											<div class="btn-group-vertical" role="group">
												<a class="btn btn-success" onclick="accessplug('<?php echo ($Plug["pid"]); ?>')">通过</a>
							...
							function accessplug(id){
								if(confirm('你确定要通过此插件？')){
									$.ajax({
										url: '<?php echo U('/Admin/Index/AccPlugin');?>',
										async: true,
										type: 'POST',
										dataType: 'json',
										data: {id:id},
