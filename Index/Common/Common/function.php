...
function getCatelogue($id){
	$catelogue = M('Tags')->where(['mode' => 1, 'tid' => $id])->find();
	return $catelogue['content'];
}
...
function getCanSeeName($data){
	if(empty($data['data']['nickname']) && empty($data['data']['username'])){
		$data = M('User')->where(['UID' => $data['uid']])->select();
		$data = $data[0];
	}
