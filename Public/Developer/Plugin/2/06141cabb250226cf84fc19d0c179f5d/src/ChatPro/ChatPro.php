...
class ChatPro extends PluginBase implements Listener{
	...
	public function onEnable(){
		...
		if($plugin = $this->getServer()->getPluginManager()->getPlugin("MGamesAPI") instanceof Plugin){
			$this->MGamesAPI = true;
			$this->getLogger()->info(TextFormat::YELLOW."连接插件MGamesAPI成功 !");
		}
	...
	public function onCmdandChat(PlayerChatEvent $event){
		...
		if(isset($this->mute[$id])){
			$player->sendMessage("[ChatPro] 你被管理员禁言了，无法发言-_-#");
		...
		if($this->factionspro->getPlayerFaction($user) == null){
			...
			$gh = "§4[{$this->factionspro->getPlayerFaction($user)}§4]";
		}
		...
		$viplevel = $vip->getInstance()->getVipLevel($player);
		if($viplevel == 0){
			$viplevel = '';
		} else {
			$viplevel = '§d[VIP '.$viplevel.' 级]';
		}
		$msg = "§d➹ §e".$user.":{$gh}{$sm}{$viplevel}§6[".$ch.">> §b".$m;
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args){
		...
		switch($command->getName()){
			case "setch":
				...
				if($conf==null){$sender->sendMessage("该玩家不存在"); return true;}
				$p = $this->getServer()->getPlayer($user);
				if($p instanceof Player){
				$this->setNameTag($p);		
				$p->sendMessage("[ChatPro] 你的称号被修改为 $ch");}
				$this->conf->set($id,$ch);
				$this->conf->save();
				$sender->sendMessage("[ChatPro] 成功修改玩家 $user 称号为 $ch");
				return true;
				}
			case "mute":
			    if(isset($args[0])){
				...
				if($p instanceof Player){
				$p->sendMessage("[ChatPro] ".$pmsg);}
				$sender->sendMessage("[ChatPro] ".$msg);
				return true;
				}
				return false;
		}
