<?php if (!defined('THINK_PATH')) exit();?>function message(type, title, txt) {
		setTimeout(function() {
			toastr.options = {
				closeButton: true,
				progressBar: true,
...
function InputCheck(form){
	...
	$(form).find('input.uk-form-danger').keyup(function(){
		if($(this).val() !== ""){
...
function umessage(msg, json){
	msg = msg.replace(/<i.*\><\/i>/g,'');
	message(json.status,'',msg);
}
...
jQuery(document).ready(function($) {
    ...
    $("#SendServer").click(function(){
        ...
        if(InputCheck('#ServerData')){
            var AjaxData;
            AjaxData = $("#ServerData").serialize();
            $.ajax({
                url: $('#ServerData').attr('action'),
                async:true,
                type: 'POST',
                dataType:'json',
                data:AjaxData,
...
function DelCartOnce(SCID){
    SCID = Number(SCID);
    $.get('<?php echo I('/User/AddCart');?>',{'do':'del','scid':SCID},function(data){
        if(data == true){
            umessage("删除成功！",{ status:'success',timeout:1000});
        }else{
            umessage("删除失败！",{ status:'danger',timeout:1000});
        }
    });
    return true;
}