...
class UserController extends UserifloginController {
    ...
    public  function Action(){
        if($_GET['var'] and $_GET['email']){
            ...
            if($Check == $Email){
                $Data['Email'] = $Check;
                $DoIt = M('User')->where($Data)->setField('Action','1');
                if($DoIt !== false){
                    $this->success('激活成功！',U('/Index/index'),3);
                    exit;
                }else{
                    $this->error('无法找到此邮箱，激活失败',U('/Index/index'),3);
                    exit;
                }
            }
        }
        ...
        您需要激活你的邮箱~<br><button class="uk-button uk-button-large uk-button-success send-email" type="button">点击发送激活邮件</button>
            ...
            jQuery(document).ready(function($){
                $(".send-email").click(function(){
                    $.ajax({
                        ...
                        type:"get",
                        beforeSend:function(){
                            $("send-email").text("发送中……").addClass("uk-button-waring").attr("disabled","");
                        },
                        success:function(data){
                            if(data["state"] == "1"){
                                ...
                                alert(data["info"]);
                                $("send-email").text("发送").removeClass("uk-button-waring").removeAttr("disabled");
                            }
                        }
                    });
                });
        ...
        if(I('get.send') == 'yes'){
            if(!S($UserSystemData['uid'])){
                ...
                $this->ajaxReturn(array('state'=>'0','info'=>'你发送的也忒急了吧~'));
                exit;
            }
            $CheckData = AuthCode($UserSystemData["email"],'ENCODE',$UserSystemData["email"],605);
            $EmailContent = '<div class="" id="qm_con_body" style="font-family: 微软雅黑,Helvetica,Arial,sans-serif;">
...
<h3 style="margin-left:30px;"><div style="height:60px;background-color:#F4F4F4;line-height: 50px;"><img src="http://www.cattery.cn/wp-content/uploads/2014/11/11.png" style="width:50px;height:50px;float: left;"><div style="
    float: left;
">亲爱的'.$UserSystemData["username"].'</div></div></h3>
<p style="margin-left:30px;">感谢注册 <a href="'.__ROOT__.'" target="_blank">PM-Plugin</a>，请在10分钟之内点击以下链接完成安全验证，激活您的邮箱！</p>
<a href="'.C('HOST').'User/Action?var='.urlencode($CheckData).'&email='.$UserSystemData["email"].'" style="margin-left:30px;" target="_blank">'.C('HOST').'User/Action/?var='.urlencode($CheckData).'&email='.$UserSystemData["email"].'</a><br>
...
<p style="margin-left:30px;">PM-plugin</p>
</div>
</div>
</div></div>';
...
$returnMail = CJson($returnMail,true);
    ...
    public function bought(){
        $UserSystemData = $this->UserSystemData;
        $this->assign('UserSystemData',$UserSystemData);
        $this->title = C('TITLE',null,"用户中心-已购商品");
        $this->ActionName = __ACTION__;
        $this->ActionTitle = '我的宝贝';
        // 进行分页数据查询 注意page方法的参数的前面部分是当前的页数使用 $_GET[p]获取
        $Num = 10;
    ...
    public function ChanceIp(){
        ...
        if(!is_numeric($SID) or  !CheckIP($Ip)){
            $this->ajaxReturn(array('state'=>'0','info'=>"数据非法"));
            exit;
        }
        $ShopData = $UserSystemData['hasshop'];
        if($ShopData[$SID]['date'] !== date("Y-m-d")){
    ...
    public function BuyShop(){//用来接收ajax提交购买商品信息
        ...
        for($i = 1;$i <= $Num;$i++){
            ...
            if($UserTmp = $User->where("uid = {$UserSystemData['uid']}")->find()) {
                //判断余额
                if (1==1/*(float)$UserTmp['money'] >= (float)$Shop['price']*/) {
                    if (AddCostRecord($Shop['price'],$UserSystemData['uid'],0,$Shop['uid'],1,"扣除购买[{$Shop['shopname']}]费用")) {//扣除消费者金额
                        //计算手续费，然后给开发者余额
                        ...
                        if(AddCostRecord($MoneyData['Fee'],0,1,$UserSystemData['uid'],1,"用户[{$UserSystemData['username']}]购买商品[{$Shop['shopname']}]手续费") and
                            AddCostRecord($MoneyData['Other'],$Shop['uid'],1,$UserSystemData['uid'],1,
                                ...
                                and AddShop($UserSystemData,$Shop['sid'],$Shop['shopname'],1)){
                            $User->commit();//直接出去
                        }else{
                            $User->rollback();
                            $this->ajaxReturn(array('state'=>'0','info'=>'程序繁忙，请重试。'));exit;
