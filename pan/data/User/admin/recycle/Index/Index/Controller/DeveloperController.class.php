...
class DeveloperController extends DeveloperifloginController {
    ...
    public function AddPlugin(){
        ...
        //数据判断:START{更新插件和新建插件的判断}
			...
			$SID = I('post.sid');
            $EditPlugin = false;
            if(I('post.checkedit') == 'true' and $SID !== false) {$EditPlugin = true;};//判断是否为更新：是EditPlugin为true
            //更新判定结束：END
        //初始数据初始化结束END
        //if(I('post.Price') < 5){$this->ajaxReturn(array('error'=>'价格请高于5元！'));exit;}
        if(I('post.QQ') =='' or I('post.QQ')=='0'){$this->ajaxReturn(array('error'=>'请填写联系方式'));exit;}
        if(M('Shop')->where($Some)->find() and $EditPlugin !== true){$this->ajaxReturn(array('error'=>'服务器已经存在！'));exit;}
        //数据判断:END
        //数据构造START
        $Shop = D('Shop');//构建自动验证的Model
        ...
        if($EditPlugin){
            ...
            $ifOwner = M('Shop')->where($SPlugin)->select();//看一下这个SID的商品是不是这个UID用户拥有的
            if($ifOwner==false){
        ...
        if($EditPlugin){//是编辑的操作
            $Plugin = M('Shop')->where("SID = '{$SID}'")->find();
            if($Plugin['pluginstate']!=='1'){
                ...
                $NewPlugin = M('Shop')->where("OldPlugin = '{$SID}'")->find();
...
HTML;
	...
	public  function NewPlugin(){
        header("Content-Type:text/html; charset=utf-8");
        $this->title = C('TITLE',null,"版主中心-添加插件");
        $UserSystemData = $this->UserSystemData;
        $this->assign('UserSystemData',$UserSystemData);
        $this->ActionName = __ACTION__;
        $this->ActionTitle = '发布插件';
        $Check = md5(md5(time()).$UserSystemData['UID']);//隐藏域用的一个检测多开编辑页面的东西
		$this->assign($url, U('/Developer/AddServer'));
        trace(I('session.'));
        session($Check,'NULL');
        $this->display();
    }
