 ...
 * @return bool，是否成功
 */
function AddShop(&$Data,$SID,$ShopName,$Num = 1){
	require('ThinkPHP/Library/Org/Tool/MulticraftAPI.class.php');
    $UserData = &$Data;
	...
	if($api){
		$serverId = $api->createServer('MC服务器：'.date('Y-m-d',strtotime('+'.$Shop['datelimit'].' day')),0,'',intval($Shop['playersolt']));
		//$serverId = array();
		//$serverId['data']['id'] = 78;
		$api->updateServer($serverId['data']['id'], array('memory'),array($Shop['memarylimit']));
		$serverConfig = $api->getServer($serverId['data']['id'])['data'];
		$i = 0;
		foreach(array_keys($api->findUsers('name', '='.$UserData['username'])['data']['Users']) as $uid){
			$i = $uid;
		}
		if($i == 0){
			$serverUid = $api->createUser($UserData['username'], $UserData['email'], 'password')['data']['id'];
		} else {
			$serverUid = $i;
		}
		$api->setServerOwner($serverId['data']['id'],$serverUid);
	}
    if($UserData && $Shop && isset($serverConfig['Server'])){
		
        for($i=0;$i<$Num;$i++){
			M('Shop')->where("sid = {$SID}")->cache(true)->setField('Num',intval($Shop['num']-1));
			unset($b);
            $UserData['hasshop'][] = array('sid'=>$SID,'shopname'=>$ShopName,'username'=>$UserData['username'],'ip'=>$Shop['apiurl'],'port'=>$serverConfig['Server']['port'],'ifuse'=>'0','stop'=>'0','serverid'=>$serverId['data']['id'],'datelimit'=>date('Y-m-d',strtotime('+'.$Shop['datelimit'].' day')));
        }
        $Update = CJson($UserData['hasshop']);
